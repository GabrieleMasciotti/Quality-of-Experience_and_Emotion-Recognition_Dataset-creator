...
def signal_processing_module(stream, seconds_per_return=1): #SAMPLES PER RETURN = SPAN = SAMPLING_RATE * PREDICTION_RATE
    # Starting the streaming phase
    ppg_to_hr = PPGtoHRAlgorithm(sampling_rate=SAMPLING_RATE, number_of_beats_to_average=1, use_last_estimate=1).ppg_to_hr
    span = SAMPLING_RATE * seconds_per_return
    residual = {'timestamp': [], 'HR': [], 'EDA': []}
    n_samples = 0
    for n, reads in stream():
        if n > 0:
            n_samples += n
            reads['HR'] = [ppg_to_hr(ppg, reads['timestamp'][i] * 1000)[0] for i, ppg in enumerate(reads['PPG'])]
...
class PPGtoHRAlgorithm:
    def __init__(self, sampling_rate, number_of_beats_to_average, use_last_estimate):
        ...
        self._set_parameters(sampling_rate, number_of_beats_to_average, self._default_buffer_size)
    ...
    def _compute_heart_rate(self):
        ...
        if self._ppg_data[self._ppg_data_buffer_size - 1] > self._ppg_data[self._ppg_data_buffer_size - 2] and \
                self._ppg_data[self._ppg_data_buffer_size - 1] > self._threshold:
            ...
            if self._climbing:
                self._peak_timestamps.append(self._timestamps[self._ppg_data_buffer_size - 2])
                self._peak_ppg_data.append(self._ppg_data[self._ppg_data_buffer_size - 2])
    ...
    def _calculate_slope(self):
        self._slope = -abs(2.0 * (self._value_peak - self._value_mean) / (4.5 * self._pulse_period))

    def _calculate_threshold(self):
        self._threshold = self._value_peak + self._number_of_samples_since_peak * self._slope
    ...
    def _remove_false_peaks(self):
        ...
        while continue_loop:
            if len(self._peak_ppg_data) < 2 or index >= len(self._peak_ppg_data) - 2:
                ...
                timestamp_diff = self._peak_timestamps[index + 1] - self._peak_timestamps[index]
    ...
    def _calculate_heart_rate(self, number_peaks):
        inter_beat_intervals = self._subtract_two_lists(
            self._peak_timestamps[number_peaks - self._number_of_beats_to_average - 1: number_peaks - 1],
            self._peak_timestamps[number_peaks - self._number_of_beats_to_average - 2: number_peaks - 2])
        ...
        ibi_test_buffer = self._subtract_two_lists(
            self._peak_timestamps[number_peaks - self._IBI_test_buffer_size: number_peaks],
            self._peak_timestamps[number_peaks - self._IBI_test_buffer_size - 1: number_peaks - 1]
        )
        ...
        if hr_median * 0.3 < hr_range:
            ...
            if self._heart_rate > self._hr_upper_limit or self._heart_rate < self._hr_lower_limit:
                if self._use_last_estimate:
                    self._heart_rate = self._last_known_hr
                else:
                    self._heart_rate = INVALID_RESULT
